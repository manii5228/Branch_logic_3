...
def signup():
    if request.method == 'POST':
        ...
        # Only allow jobseeker role for signup
        role = 'jobseeker'
...
def schedule_interview(application_id):
    ...
    # Calculate min datetime for scheduling (applied_at + 1 hour, or now + 1 hour if missing)
    from datetime import timedelta
    if application.applied_at:
        min_dt = application.applied_at + timedelta(hours=1)
    else:
...
@login_required
def job_alerts():
    # Allow user to set or update job alert preferences
    ...
    # Find jobs matching alert preferences
    jobs = []
    if alert and (alert.keywords or alert.location):
        jobs_query = Job.query.filter_by(status='active')
        if alert.keywords:
            jobs_query = jobs_query.filter(Job.title.ilike(f"%{alert.keywords}%"))
        if alert.location:
...
@login_required
def career_insights():
    # Only include jobs posted by admin users
    from sqlalchemy import func
    admin_users = db.session.query(User.id).filter_by(role='admin').subquery()
    top_titles = (
        db.session.query(Job.title, func.count().label('count'))
        .filter(Job.employer_id.in_(admin_users))
        .group_by(Job.title)
